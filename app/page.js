...
export default function Home() {
  ...
  // Training accounts get a tightly restricted landing page regardless of what
  // staff_roles they hold — only the modules that have actually been verified as
  // demo-data-isolated are shown. Fees & Bills, Communication, Reports, Registers
  // Not Done and Mentor Groups are deliberately excluded: either their schema was
  // never captured in this repo (fees/reports) or they touch real, unscoped data
  // (communication genuinely emails real people — see migration 081), so hiding
  // the tile stops an accidental click even though the real safeguard is server-side.
  if (profile?.is_demo_account) {
    return (
      <div>
        <div className="card" style={{ borderLeft: '4px solid #c07d1f', background: '#fff7e0' }}>
          <strong>Training account.</strong> Everything below is fake practice data that resets automatically — nothing you do here affects real students. Fees & Bills, Communication and Reports aren't available on this account yet.
        </div>
        <h1>Welcome — Training Account</h1>
        <div className="module-card-grid">
          <ModuleCard
            icon="🏠" label="Dashboard" accent="myinfo"
            description="Your timetable, calendar and inbox."
            items={[
              { href: '/staff/timetable', label: 'My Timetable' },
              { href: '/calendar', label: 'Calendar' },
              { href: '/inbox', label: 'Inbox' },
          ...
          />
          <ModuleCard
            icon="🎓" label="Students" accent="students"
            description="Core records, behaviour, attendance, results and certificates — all practice data."
            items={[
              { href: '/students', label: 'Core Data' },
              { href: '/behaviour', label: 'Behaviour' },
              { href: '/attendance', label: 'Attendance' },
