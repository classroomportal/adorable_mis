...
function AttendanceMark({ status, code }) {
...
// the figure that actually means something in a pastoral conversation.
...
function StudentDetail() {
  ...
  // Today's row in the register, read as the student's day: every lesson they
  // are timetabled for, marked or not, plus any mark taken in a period with no
  // timetabled lesson (evening prep, a cover arrangement). An unmarked lesson
  // is as worth seeing as a marked one — it is the register nobody took.
  const todayDayLabel = new Date().toLocaleDateString('en-GB', { weekday: 'short', timeZone: SCHOOL_TIMEZONE });
  const todayMarks = Object.fromEntries(attendanceToday.map((a) => [a.period_number, a]));
  const todayLessons = periods
    .map((p) => {
      const cell = cellMap[`${todayDayLabel}-${p.period_number}`];
      const mark = todayMarks[p.period_number];
      if (!cell && !mark) return null;
      return {
        ...
        {attendanceSummary.length === 0 && attendance.length === 0 ? <p>No attendance recorded.</p> : (
          <>
            <div className="att-scopes">
              {attendanceSummary.map((row) => <AttendanceScopeCard key={row.scope} row={row} />)}
            </div>
            ...
            {attendance.length === 0 ? <p>No marks recorded yet.</p> : (
              <div className="table-scroll">
                <table>
