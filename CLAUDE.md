...
- Nova-T imports resolve a class's subject **only** through `subjects.subject_code`, matched against the group code's suffix with the set number stripped (`10LI/El` and `12a/El1` both reduce to `El`). `subject_code` is editable from `/admin/subject-settings` (the "Nova-T code" column), so when adding a subject that will appear in a Nova-T export, set its code there — a subject with no code is never matched by a class import, and a wrong code silently reassigns real classes on the next one. The importer matches case-insensitively, and that page rejects a save that would put the same code on two subjects. The original import (`sql/013`) invented subject names by *guessing* what each code stood for and got `El` wrong — it expanded the school's Literature code to "Electronics" (see migrations 117–118). Before trusting any subject that import created, check it against who teaches it and where. Don't re-derive subject names from codes; the codes are the school's, not guessable.
