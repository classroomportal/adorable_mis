...
export async function generateTranscript(studentId, termId = null) {
  ...
  // or explicitly tagged for this student's key stage.
  ...
  // Biology if they're only enrolled in combined Science) AND matches
  // their key stage.
  const subjectNameById = {};
  (results || []).forEach((r) => {
    if (!enrolledSubjectIds.has(r.subject_id) || !matchesKeyStage(r.subject_id)) return;
    subjectNameById[r.subject_id] = r.subjects?.display_name || r.subjects?.subject_name || `Subject ${r.subject_id}`;
  });
  (targets || []).forEach((t) => {
    if (!enrolledSubjectIds.has(t.subject_id) || !matchesKeyStage(t.subject_id)) return;
