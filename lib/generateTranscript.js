...
function buildWeekColumns(term) {
  const weeks = [];
...
export async function generateTranscript(studentId) {
  ...
  // Collect every subject that appears in either results or targets.
  ...
  const filename = `${(student?.last_name || 'student').toUpperCase()}-${student?.student_id}-Transcript.pdf`;
  doc.save(filename);
}
