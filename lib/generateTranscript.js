...
// Builds the transcript jsPDF document in memory without doing anything with
// it — shared by the live "Download Report" button (generateTranscript,
// below) and the staff publish flow (publishTranscript) so both paths render
// an identical PDF from a single implementation.
async function buildTranscriptDoc(studentId, termId) {
  ...
  // Collect every subject that appears in results (and matches the
  // student's key stage). Targets alone no longer earn a subject a row —
  // a subject with a target but no results yet shouldn't show up on the
  // transcript.
  ...
  doc.setFontSize(16);
  doc.text('Report', 148, 18, { align: 'center' });
  ...
  for (const term of terms || []) {
    ...
    const body = subjectIds.map((sid) => {
      ...
      (results || [])
        ...
        .forEach((r) => {
          ...
          if (!label) return;
          const marker = r.result_type === 'exam_grade' || r.result_type === 'term_exam_import' ? '*' : '';
    ...
    const categoryHead = [['Week', 'Data type']];
    const categoryBody = weeks.map((w) => [w.label, Array.from(categoryByWeek[w.label]).join(' / ') || '—']);

    if (cursorY > 170) { doc.addPage('landscape'); cursorY = 20; }

    doc.setFontSize(12);
    doc.text(`${term.term_name} (${formatUKDate(term.start_date)} – ${formatUKDate(term.end_date)})`, 12, cursorY);
    ...
    autoTable(doc, {
      ...
      body: categoryBody,
      theme: 'grid',
      styles: { fontSize: 6, cellPadding: 1.2, lineColor: [60, 60, 60], lineWidth: 0.3, textColor: [90, 90, 90] },
  ...
  doc.setFontSize(8);
  doc.text('* = Exam grade. Unmarked = ReLP / Short Test / Teacher Assessment (weekly assessment).', 12, 200);

  return { doc, student, terms };
}

export async function generateTranscript(studentId, termId = null) {
  const { doc, student, terms } = await buildTranscriptDoc(studentId, termId);
...
export async function publishTranscript(studentId, termId = null) {
  const { doc, terms } = await buildTranscriptDoc(studentId, termId);
  const blob = doc.output('blob');
  const path = `${studentId}/transcript-${termId || 'all'}.pdf`;
  ...
  const title = termId && terms?.[0] ? `Transcript — ${terms[0].term_name}` : 'Transcript — All terms';
  const payload = {
  ...
  // A plain upsert(onConflict:...) can't reliably match an existing row when
  // term_id is null (NULL never equals NULL for uniqueness purposes), so the
  // "overwrite the previous copy" behaviour is done explicitly here instead.
