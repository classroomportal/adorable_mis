...
export async function generateInvoicePdf({ student, term, lineItems, payments, invoiceStatus }) {
  ...
  autoTable(doc, {
    startY: 52,
    head: [['Item', 'Amount']],
    body: (lineItems || []).map((li) => [li.description || li.fee_items?.name || '-', naira(li.amount)]),
...
export async function generateInvoicePdfForStudent(studentId, termId) {
  ...
  const [{ data: lineItems }, { data: payments }] = await Promise.all([
    supabase.from('invoice_line_items').select('description, amount, fee_items(name)').eq('invoice_id', invoice.id).order('created_at'),
