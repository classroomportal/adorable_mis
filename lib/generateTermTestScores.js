...
async function buildTermTestScoresDoc(studentId, termId) {
  ...
  async function subjectsForTerm(weeks, assessments, assessedByWeek) {
  ...
  for (const term of terms || []) {
    ...
    const { subjectIds, subjectNameById } = await subjectsForTerm(weeks, assessments, assessedByWeek);
    ...
    const body = subjectIds.map((sid) => {
      ...
      weeks.forEach((w) => {
        if (cellByWeek[w.label]) { row.push(cellByWeek[w.label]); return; }
        const assessed = assessedByWeek[w.label];
        if (assessed && !assessed.has(sid)) {
          anyShaded = true;
